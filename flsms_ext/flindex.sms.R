...
setClass("FLIndex.SMS",
    ...
    representation(
     ...
     prototype=prototype(range.SMS=list(season=1, power.age=-1, q.age=0,
        var.age.group=as.vector(0,mode="list"),minCV=0.3,species=1, sp.fl=1,fleet.no=1))
)
...
FLIndex.SMS <- function(name=character(0), desc=character(0), distribution=character(0),
    ...
    q.age=NA,var.age.group=NA,minCV=0.3,species=1,sp.fl=1, fleet.no=1, ...) {
    ...
    new <- new("FLIndex.SMS", name = name, desc = desc, distribution = distribution,
        ...
        range.SMS = unlist(list(season=season, power.age=power.age,
                 q.age=q.age,var.age.group=var.age.group, minCV=minCV,species=species,sp.fl=sp.fl,fleet.no=fleet.no))
    ...
    for (sp in 1:nsp) {
     for (fl in 1:n.fleet[sp]) {
      ...
      indx@range.SMS<-list("season"=seas, "power.age"=la.p,
                            "q.age"=la.q,"var.age.group"=as.vector(fl.var[v:(v-1+n.var)]),"minCV"=minCV,"species"=sp,"sp.fl"=sp.fl,fleet.no=fl)
...
FLIndices2SMS<-function(out.path=NULL,indices=NULL,control=NULL,fleet.inf="fleet_info.dat",
    ...
    cat("#############",
        "\n#Type of index",
        " \n#   1= effort and  catch number at age."
        " \n    2= effort and  sum of exploitable biomass (including all age with F>0 XXXXXinput).",
        " \n    3= effort and  sum of spawning stock biomass",
        " \n    4= total commercial effort and one column with dummy values. Fbar is assumed proportional to effort",
