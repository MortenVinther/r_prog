...
setClass("RSMS.control",
  ...
  prototype=prototype(
        ...
        incl.process.noise=as.vector(0,mode="list"),
    ...
    switch(x,
           ...
           "combined.catches"  ={if (nice) {
             ...
             cat("## use seasonal or annual catches",inpOnly,  "(option combined.catches)\n",
                 '#    0=annual catches in liklihood, with seasonal proportions of annual F at age derived from seasonal catch N / sum(all seasonal catch N)\n',
                 ...
                 '#    3=seasonal catch in likelihood with estimation of seasonal F from separable F-model (not implemented yet \n',
                 file=file,append=T,sep="")   
             if (expand) wr.vector.expand(slot(control,x),VPA.species) else wr.vector.nice(slot(control,x),VPA.species)
           } else  cat(slot(control,x),"\t#",x,"\n",file=file,append=TRUE)
           }, 
...
read.RSMS.control<-function(dir='.',file="rsms.dat",test=FALSE) {       
