...
my.colors<-unlist(mget("MyPalette",ifnotfound =list(my.colors)))

palette(my.colors)

#rm(stom)
if (!(exists("stomach.dir"))) stomach.dir<-data.path
if (!(exists("stomach.dir.oldNew"))) stomach.dir.oldNew<-data.path
...
ll<-Read.stom.size.classes() %>% mutate(Quarter=paste0('Q',quarter),species=NULL,quarter=NULL)
stom<-left_join(x=stom,y=ll,join_by(Predator.no == species.no,Predator.length.class==size.class,Year==year,Quarter)) %>% rename(Predator.sc=size)
stom<-left_join(x=stom,y=ll,join_by(Prey.no == species.no,Prey.length.class==size.class,Year==year,Quarter)) %>% rename(Prey.sc=size)
...
if (FALSE) a<-by(stom,list(stom$Quarter,stom$Year,stom$Predator.no),function(x) {
   if (dim(x)[[1]]>0) {
    ...
    if ((i %% (nox*noy-1))==0) {
      ...
      par(mar=c(3,5,3,2)) 
      plot.new(); legend(x=0,y=1,all.names,fill=all.prey.col,cex=my.cex,ncol=2)
    }    
    i<<-i+1
  ...
  compare_stoms<-function(stom) {
    ...
    if (TRUE) by(stom,list(stom$Quarter,stom$Year,stom$Predator.no),function(x) {
      ...
      if (oldY != newY | oldNo !=newNo) {
        ...
        plot.new();
        title(main=list(paste0("\nupper:",stomLabes[2],"  lower:",stomLabes[1]),cex=my.cex*1.0),outer=TRUE)
        legend("topleft",all.names,fill=all.prey.col,cex=my.cex*1.0,ncol=2)
        oldY<<-newY  
        oldNo<<-newNo
      }
...
compare_stom_all(out.dir=stomach.dir.oldNew)
...
oldP<- -9
 ...
 xx<- by(stom,list(stom$Quarter,stom$Year,stom$Predator.no),function(x) {
    
   newY<<-x[1,'Year']
   newP<<-x[1,'Predator.no']
   cat(newY,newP,x[1,'Quarter'],'\n')
    ...
    if (oldY != newY | oldP != newP) {
     ...
     # text(x=0.0,y=0.07,"lower: observed",pos=4)
      plot.new();
      title(main="upper: expected\nlower: observed",cex=my.cex*2)
      #print(paste(x[1,]$Predator,x[1,]$Year,sep='-'))
...
if (make_compositional_residuals) {
  ...
  a<- stom %>% dplyr::select(split,Area,Year,Quarter,Predator.no,Predator,Predator.length.class,Predator.sc,Predator.length,N.samples,Size.model,Prey.no,Prey,Diri.like,Diri.sum.p,Diri.p,stomcon,stomcon.hat,pl.res,Residual,stan.residual)
  a<-filter(a,Diri.p>0)
  ab<-left_join(a,b) %>% mutate(split=NULL)
  
  head(ab)
} else {
  ab<- stom %>% dplyr::select(split,Area,Year,Quarter,Predator.no,Predator,Predator.length.class,Predator.sc,Predator.length,N.samples,Size.model,Prey.no,Prey,Diri.like,Diri.sum.p,Diri.p,stomcon,stomcon.hat,pl.res,Residual,stan.residual) %>% mutate(cres=0)
}
...
if (FALSE) {
  if (make_compositional_residuals) head(ab %>% arrange(desc(abs(cres))),n=20) %>% dplyr::select(-Size.model,-Area,-Predator.length,-Predator.length.class,-Predator.no,-Prey.no) %>% as_tibble()
  head(ab %>% arrange(desc(abs(pl.res))),n=20) %>% dplyr::select(-Size.model,-Area,-Predator.length,-Predator.no,-Predator.length.class,-Prey.no)%>% as_tibble()
  head(ab %>% arrange(desc(abs(stan.residual))),n=20) %>% dplyr::select(-Size.model,-Area,-Predator.length,-Predator.no,-Predator.length.class,-Prey.no)%>% as_tibble()
 head(ab %>% arrange(desc(Diri.like)),n=20) %>% dplyr::select(-Size.model,-Area,-Predator.length,-Predator.no,-Predator.length.class,-Prey.no)%>% as_tibble()
 head(ab %>% arrange(desc(abs(cres))),n=20) %>% dplyr::select(-Size.model,-Area,-Predator.length,-Predator.no,-Predator.length.class,-Prey.no)%>% as_tibble()
...
xx<-by(b,list(b$Predator,b$Prey,b$Area),function(x) {
  if (i==noxy) { newplot(dev,nox,noy,filename=paste0('aa_stom_box_',x[1,'Predator']),dir=stom.resid.dir.raw_resid); i<<-0 ;  par(mar=c(3,2,3,2)) }#c(bottom, left, top, right)
