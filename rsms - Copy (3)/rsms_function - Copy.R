...
func <- function(parameters) {
 ...
 seasFprop[['SSA']][,2,]<-1
  ...
  for (s in 1:nSpecies) {
    ...
    } else if (useRho[s]==1) 2 = {  # AR(1)
         fcor<-pow(rho,abs(matrix(1:stateDimF[s] - 1, nrow = stateDimF[s], ncol = stateDimF[s], byrow = TRUE) - 1:stateDimF[s] - 1))
         fvar <- outer(1:stateDimF[s],
                       1:stateDimF[s],
                       function(i,j)fcor[cbind(i,j)]*fsd[i]*fsd[j])  
 ...
 # 
   ...
   if (sms.mode>0) {
     ...
     if (dim(overlapIdx)[[1]]>0)  for (ii in (1:dim(overlapIdx)[[1]])) {overlap[overlapIdx[ii,"q"],overlapIdx[ii,"predNo"],overlapIdx[ii,"preyNo"]] <- overlapP[overlapIdx[ii,'overlap']]}
  ...
  # Stomach contents observations
     ...
     for (yqa in (1:min(dim(stom)[[1]],2000))) {
       ...
       for (yqapa in (1:dim(x1$data[[1]])[[1]])) {
         ...
         stomVar <- exp(stomObsVar[x2$stomObsVarIdx])
      ...
      # if (pred==8 & y==40 & q==3 & predSize %in% c(12,13)){
      #     cat('stomconTot:' ,stomconTot,'\n')
      #     cat('EstomTot:\n')
      #     print(EstomTot)
      #   
      #      print( log(stomconTot)-log(EstomTot))
      #      print( sum(dnorm(log(stomconTot),log(EstomTot),sd=sqrt(stomVar),log=TRUE)))
      # }
