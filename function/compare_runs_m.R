...
compare_runs_M<-function(
  ...
  run.ID='SMS_M_',         # file id used for paper output
  doGrid=TRUE,
  ...
  for (dir in dirs) {
    ...
    if (verbose) cat(dir,'\n')
     ...
     a<-subset(a,(Year>=first.year.on.plot & Year<=last.year.on.plot & Z>0),
               select=c(Species, Year,Age,M,M1,M2))
     if (sum(a$M2)>1) a$M<-a$M1+a$M2; 
     a$M1<-NULL; a$M2<-NULL
    ...
    if (verbose) cat(dir,'\n')
     ...
     a$M1<-NULL; a$M2<-NULL
      a<-data.frame(scenario=labels[which(dirs==dir)],Variable="M",Year=a$Year, Species=a$Species, Age=a$Age,Z=a$Z,
           dead=a$N.bar*a$Z,deadm=a$N.bar*(a$M))
   ...
   if (sp %in% dimnames(values)[[3]]) {
    ...
    if (maxval>0) {
      if ((gi %% (nox*noy))==0  | gi==0) {
        filename<-paste0("compare_M_",run.ID,'_',sp)
        ...
        legend("center",legend=labels,col=first.color:(first.color+len.dir-1),
              pch=first.pch:(first.pch+len.dir-1),cex=2,title=paste0('M: ',sp))
        gi<<-gi+1
