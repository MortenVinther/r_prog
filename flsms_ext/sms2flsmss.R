...
SMS2FLSMSs<-function(sumfile=file.path(data.path,'summary.out'),residualsFile=file.path(data.path,"catch_survey_residuals.out"),bio.interact=FALSE,read.input=FALSE,read.output=TRUE,
...
if (FALSE) {
  # first run init.R to set environment etc and get SMS.control
  data.path
  a<-SMS2FLSMSs(#sumfile=file.path(data.path,'summary.out'),
             #residualsFile="catch_survey_residuals.out",
             bio.interact=FALSE,read.input=TRUE,read.output=TRUE,
             control=SMS.control,FLStocksMulti=NULL) 
