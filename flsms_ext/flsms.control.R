    ...
    switch(x,
       ...
       "species.info"        = {    ncols<-11
                                   ...
                                   for (s in (1:nsp)) { 
                                     ...
                                     if(tmp[s,7]==2 | tmp[s,7]==3) n.oth.pred<-n.oth.pred+1;
                                   }
...
trunc.FLSMS.control<-function(control,inclVPA=1) {
  nVPA<-length(inclVPA)
  stopifnot(all(inclVPA>=first.VPA))
  oc<-control
  n.<-slotNames(control)
  for (x in n.) {
    switch(x,
           "no.species"          = {slot(control,x)<-as.integer(nVPA);
           spR<-inclVPA 
           spR2<-(inclVPA-first.VPA+1);
           species.names<-sp.names[spR]
           },
           "species.names"       = { slot(control,x)<-species.names; },
           "species.info"        = {  
                                     if (nVPA==1) slot(control,x) <-t(as.matrix(slot(oc,x )[spR,],nrow=nVPA)) 
                                     if (nVPA>1) slot(control,x) <-slot(oc,x )[spR,]},       
           "beta.cor"            = {slot(control,x)<-slot(oc,x)[spR2]},
           "SSB.R.year.first"    = {slot(control,x)<-slot(oc,x)[spR2]},
           "SSB.R.year.last"     = {slot(control,x)<-slot(oc,x)[spR2]},
           "obj.func.weight"     = {   if (nVPA==1) slot(control,x) <-t(as.matrix(slot(oc,x)[spR,],nrow=nVPA)) 
                                       if (nVPA>1) slot(control,x) <-slot(oc,x )[spR,]},  
           "discard"             = {slot(control,x)<-slot(oc,x)[spR2]}, 
           "combined.catches"    = {slot(control,x)<-slot(oc,x)[spR2]}, 
           "seasonal.catch.s2"   = {slot(control,x)<-slot(oc,x)[spR2]}, 
           "catch.s2.group"      = {slot(control,x)<-slot(oc,x)[spR2]},
           "catch.season.age"    = {slot(control,x)<-slot(oc,x)[spR2]},                                
           "avg.F.ages"          = {if (nVPA==1) slot(control,x) <-t(as.matrix(slot(oc,x)[spR2,],nrow=nVPA)) 
                                    if (nVPA>1) slot(control,x) <-slot(oc,x )[spR2,]}, 
           "min.catch"           = {slot(control,x)<-slot(oc,x)[spR2]}, 
           "catch.sep.year"      = {slot(control,x)<-slot(oc,x)[spR2]},
           "catch.spline.year"    = {slot(control,x)<-slot(oc,x)[spR2]},
           "fix.F.factor"        = {slot(control,x)<-slot(oc,x)[spR2]},
           # otherwise              
           {}                                 
    )}
  control
