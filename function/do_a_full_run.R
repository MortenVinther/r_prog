...
do.a.full.SMS.run<-function(label="",rundir=data.path,outdir=data.path,do.single=TRUE,redo.single=TRUE,do.multi.1=FALSE,do.multi.2=FALSE,do.multi.2.redo=FALSE, do.multi.2.redo.Nbar.fixed=FALSE,do.multi.2.redo.Nbar=FALSE,
...
if (do.multi.2) {
    ...
    if (do.hessian &  do.multi.2.redo==FALSE & do.multi.2.redo.Nbar==FALSE & do.multi.2.redo.Nbar.fixed==FALSE) hes.string<-' ' else hes.string<-' -nohess '
    if (do.MCMC & do.hessian & do.multi.2.redo==F & do.multi.2.redo.Nbar==FALSE & do.multi.2.redo.Nbar.fixed==FALSE) hes.string<-paste(" -mcmc",as.integer(mcmc)," -mcsave",mcsave)
    if (OS=="windows") cat(paste(dosDrive,"\n","cd ",quo,outdir,quo,"\n",sep=""),file=sms.do,append=append)
   if (cleanup) cat(paste(del,quo,"sms.rep \n",sep=""),file=sms.do,append=TRUE)
   if (cleanup) cat(paste(del,quo,"sms.par \n",sep=""),file=sms.do,append=TRUE)
   if (cleanup) cat(paste(del,quo,"sms.std \n",sep=""),file=sms.do,append=TRUE)
...
if (do.multi.2.redo) {
    ...
    if (do.hessian &  do.multi.2.redo.Nbar==FALSE & do.multi.2.redo.Nbar.fixed==FALSE) hes.string<-' ' else hes.string<-' -nohess '
    if (do.MCMC & do.hessian & do.multi.2.redo.Nbar==FALSE & do.multi.2.redo.Nbar.fixed==FALSE) hes.string<-paste(" -mcmc",as.integer(mcmc)," -mcsave",mcsave)
    if (OS=="windows") cat(paste(dosDrive,"\n","cd ",quo,outdir,quo,"\n",sep=""),file=sms.do,append=append)
   if (cleanup) cat(paste(del,quo,"sms.rep \n",sep=""),file=sms.do,append=TRUE)
   if (cleanup) cat(paste(del,quo,"sms.par \n",sep=""),file=sms.do,append=TRUE)
   if (cleanup) cat(paste(del,quo,"sms.std \n",sep=""),file=sms.do,append=TRUE)
...
if (do.multi.2.redo.Nbar | do.multi.2.redo.Nbar.fixed) {
    #run SMS in multi species mode 2 with estimation of all parameters, start to change options
    control@VPA.mode<-2                    # set multispecies mode =2
    control@phase.stom.var<- -1              # stomach variance.
