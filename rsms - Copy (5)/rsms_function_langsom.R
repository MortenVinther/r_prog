...
func <- function(parameters) {
  ...
  for (s in seq_len(nSpecies)) logSeasFprop[[s]][seasFprop[[s]]>0] <- log(logSeasFprop[[s]][seasFprop[[s]]>0])
    ...
    for(i in seq_len(timeSteps)) {
      for(j in seq_len(stateDimN[s])) {
        ...
        if (j >1 | recSeason==1 ) {  # first age might not have enter the model yet
          ...
          if((keyLogFsta[s,j])>0) {
            Zq[[s]][i,fq,j] <- exp(logFiq[[s]][i,fq,j]) + MM[[s]][i,fq,j]
          } else {
            Zq[[s]][i,fq,j] <- MM[[s]][i,fq,j]
          }
          logNbarq[[s]][i,fq,j] <- logNq[[s]][i,fq,j]-log(Zq[[s]][i,fq,j]) + log(1.0 -exp(-Zq[[s]][i,fq,j]))
          if (keyLogFsta[s,j]>0) Chat[[s]][j,i] <- exp(logNbarq[[s]][i,fq,j]+logFiq[[s]][i,fq,j])
        ...
        if (j ==1 & recSeason>1) {
           ...
           if (keyLogFsta[s,j]>0) {
             Zq[[s]][i,recSeason,j] <- exp(logFiq[[s]][i,recSeason,j]) + MM[[s]][i,recSeason,j]
           } else {
           ...
           if (keyLogFsta[s,j]>0)  {Chat[[s]][j,i] <-  exp(logNbarq[[s]][i,recSeason,j]+logFiq[[s]][i,recSeason,j])
           } else { Chat[[s]][j,i]<-0.1  }
        }
        ...
        if (nSeasons>1) for (q in 2:lq) if (j >1 | q>recSeason ) {
          ...
          if (keyLogFsta[s,j]>0) {
            Zq[[s]][i,q,j] <- exp(logFiq[[s]][i,q,j]) + MM[[s]][i,q,j]
          } else {
            Zq[[s]][i,q,j] <- MM[[s]][i,q,j]
          }
          logNbarq[[s]][i,q,j] <- logNq[[s]][i,q,j]-log(Zq[[s]][i,q,j]) +log(1.0 -exp(-Zq[[s]][i,q,j]))
          if (keyLogFsta[s,j]>0)  {Chat[[s]][j,i] <- Chat[[s]][j,i]+ exp(logNbarq[[s]][i,q,j]+logFiq[[s]][i,q,j])}  else { Chat[[s]][j,i]<-0.1  }
        }
      }
    }
  ...
  fleets<-keySurvey.overview[keySurvey.overview[,'s']==s,'f']
    ...
    if (surveyType[fl]==1) {
      ...
      for (a in mina[fl]:maxa[fl]) {
        ...
        obs.no<-keysA[,'obs.no']
        predSurveyObs[obs.no]<- logFiq[[s]][flYears,q,a] + logCatchability[keyCatchability] 
      ...
      for (n in 1L:length(obs.no))  {
        ...
        for(j in seq.int(faf,laf)) {
           predSurveyObs[obs.no[n]]<- predSurveyObs[obs.no[n]]+ exp(logFiq[[s]][y,q,j]) #sum F within Fbar range
        }
