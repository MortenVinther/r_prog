...
transExternalData<-function(inp='summary.out',outSet='SMS_old_detailed',
  ...
  a<-read.table(file=inp,header=TRUE) %>%as_tibble() %>%
    transmute(year=Year,quarter=Quarter,age=Age,species=exSpeciesNames[Species.n],s=match(species,spNames),M2,N) %>%
    filter(!is.na(s) & M2>=0) 
  
  sms<-list(rep=list(res=a))
...
saveResults<-function(runName='nr1',data=data,parameters=parameters,obj=obj,opt=opt,lu=lu,map=myMap,random=random,rep=myRep,sdrep=sdrep) {
  ...
  sms<-list(data=data,parameters=parameters,obj=obj,opt=opt,lu=lu,map=myMap,random=random,rep=myRep,sdrep=sdrep)
