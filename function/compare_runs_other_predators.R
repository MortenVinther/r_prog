compare_runs_other_predators<-function(
  ...
  for (p in pp){
    ...
    if (nn>4) {
      a$N<-log(a$N*1000)
      ylab<-"log(stock numbers)"
    } else {
      a$N<-a$N/1000
      ylab<-'stock numbers (millions)'
    }
    print(xyplot( N~Year |paste(Predator,' Q',Quarter,' Age:',Age,sep=''),groups=label, data=a,
      type='b',lwd=2 , layout=my.layout, ylab=ylab,
       strip = strip.custom( bg='white'),par.strip.text=list(cex=1.25, lines=1.7),
