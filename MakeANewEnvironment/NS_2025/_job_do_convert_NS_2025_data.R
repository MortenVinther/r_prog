...
newEnv<-'NS_2025'
exchangeDir<-file.path( "Data_NorthSea","input_NS_2025")   # directory with data  files on spread sheet format
RexchangeDir<-"MakeANewEnvironment"     # directory with R scripts to convert files etc.
finalExchangeDir<-file.path(root,"Data_NorthSea","final_input_NS_2025")

addfn<-'key2025'  # a label for files
...
my.stock.dir<-'NS_2025_key_run'
data.path<-file.path(root,my.stock.dir)

#####################################################################################
# Then 2, read ICES assessment data and update SMS data
if (do.2) {
   source(file.path(prog.path,RexchangeDir,newEnv,"read_update_assessment_data_NS_2025.R"))
