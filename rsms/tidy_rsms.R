...
# and now surveys
  #cat("survey: ",fl,'\n')
      ...
      predObs<-logNq[[s]][keySurvey[,'y'],keySurvey[,'q'],keySurvey[,'a']] - 
                  Zq[[s]][keySurvey[,'y'],keySurvey[,'q'],keySurvey[,'a']]*sampleTimeWithinSurvey[keySurvey[,"f"]]
      if(keyPowerQ>0) predObs <- predObs*exp(logQpow[keyPowerQ])
      predObs <- predObs+logCatchability[keySurvey[,"keyCatchability"]]

      var <- varLogObsSurvey[keySurvey[,"keyVarObsSurvey"]]
      ans <- ans - sum(dnorm(logSurveyObs,as.vector(predObs),sqrt(var),log=TRUE))
