...
a<-read.csv(file=file.path(root,exchangeDir,'ByStock','sandeel','effort_to_sandeel_assessment_1982_2022_north_south.csv'))
...
head(b)
tst<-a %>% select('species','year','quarter')
duplicated(tst)
summary(b)
...
any(a %>% select('species','year','quarter','effort2','age') %>% arrange(species,year,quarter,age) %>%duplicated())
...
write.surv(sp='SSA',nr=1,effort=1E-6,digits=4)
...
write.surv(sp='SPR',nr=3,effort=1E-08,digits=0)
write.surv(sp='SPR',nr=4,effort=1E-04,digits=0)
