...
a<-read.csv(file=file.path(root,exchangeDir,'CPUE per age per subarea_2023-08-09 12_41_45.csv'))
a<-subset(a,Year>=1974)
head(a)
sort(unique(a$Species))
sort(unique(a$Quarter))
...
#summary(subset(b3,Species=="Sprattus sprattus"))
...
#######################
# re-calculate proportions without the English Channel, as data years are few
b<-subset(b,Area != 10)
