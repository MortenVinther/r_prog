...
# inclProcess[c(2:3,8)]<-2 ; inclProcess[c(5,7)]<-3; inclProcess[6]<-4 ;inclProcess 
...
for (s in 1:nSpecies) {
  ...
  if (inclProcess[s]==2) aa<-aa[1] else if (inclProcess[s]==3) aa<-tail(aa,-1) else if (inclProcess[s]==4) aa<-numeric(0)
  if (inclProcess[s]==1 | inclProcess[s]==3) {
    for (j in (1:(length(aa)-1))) {
      for (age in aa[j]:(aa[j+1]-1)) {
        x[s,age+off.age ]<-i;
...
nlogN[inclProcess==3]<-nlogN[inclProcess==3]-1L # older age only
nlogN[inclProcess==4]<-0L
