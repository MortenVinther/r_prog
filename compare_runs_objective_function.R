...
if (F) {
  ...
  dirs<-c("NS_2023_04__Simple_0001_haul_as_observed","NS_2023_04__Boots_0500_haul_mu","NS_2023_04__Boots_0500_haul_as_observed")
  labels<-c("default","alpha 0","alpha prey")
...
write.table(aaa,file=file.path(data.path,paste0("compare_like_",IDout,".csv")),sep=',',row.names=F)
...
write.table(bb,file=file.path(data.path,paste0("compare_like_species_",IDout,".csv")),sep=',',row.names=F)
...
aa<-dplyr::arrange(aa,Species.n,sp_fl,label,)
write.table(aa,file=file.path(data.path,paste0("compare_like_fleet_",IDout,".csv")),sep=',',row.names=F)
