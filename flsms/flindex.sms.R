...
setClass("FLIndex.SMS",
    ...
    representation(
     ...
     prototype=prototype(range.SMS=list(season=1, power.age=-1, q.age=0,species=1,
...
FLIndex.SMS <- function(name=character(0), desc=character(0), distribution=character(0),
    ...
    q.age=NA,var.age.group=NA,minCV=0.3,species=1, ...) {
    ...
    new <- new("FLIndex.SMS", name = name, desc = desc, distribution = distribution,
        ...
        range.SMS = unlist(list(season=season, power.age=power.age,
                 q.age=q.age,var.age.group=var.age.group, minCV=minCV,species=species))
    ...
    for (sp in 1:nsp) {
     for (fl in 1:n.fleet[sp]) {
      ...
      indx@range.SMS<-list("season"=seas, "power.age"=la.p, 
                            "q.age"=la.q,"var.age.group"=as.vector(fl.var[v:(v-1+n.var)]),"minCV"=minCV,"species"=sp)
