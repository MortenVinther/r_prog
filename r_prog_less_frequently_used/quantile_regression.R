...
first.qr.spcies.no<-9  # first predator species where qr is done  pred(9)=R. radiata
...
stom2<-subset(stom,Size.model>0 & Prey.no !=0 &stomcon>1E-4 & Predator.no>=first.qr.spcies.no)
...
by(stom2,list(stom2$Prey.no,stom2$Predator),function(x) {
 if (min(x$Predator.size) < max(x$Predator.size)) { 
   if (i==noxy) {newplot(dev,nox,noy,filename=paste0("quantile_reg_",x[1,]$Predator," eating ",x[1,]$Prey)); i<<-0 }
...
stom<-Read.stomach.data()
stom<-subset(stom,Size.model>0 & Prey.no !=0 &stomcon>1E-4  & Predator.no>=first.qr.spcies.no )
...
qunatileRegression<-function(tau=0.05){
  ...
  cat(x[1,'Predator'],"eating",x[1,'Prey'],'\n')
   try(rq(log(Predator.size/Prey.size)~log(Predator.size) ,data=x, tau=tau))
  })
  ...
  for (i in dimq[[1]]) for (j in dimq[[2]]) {
    cat("i:",i, "  j:",j," class:",class(q[[i,j]]),'\n')
   # if (!is.null(q[[i,j]])) {
    if ( class(q[[i,j]])== "rq") {
...
##############################################################
# plot pred prey size relations for all combinations of predators and preys
