func <- function(parameters) {
 ...
 # MM[[i]]<-natMor[[i]]  # natural mortality, M or M1+M2
  ...
  # 
  # #function for Fishing mortality (annual (q=0) or seasonal)
  # FiMo<-function(s,y,a,q=0L,expIt=TRUE) {
  #   if (ageQCatchMis[[s]][y,a]) {  # no catch at age in at least one season
  ...
  for (s in seq_len(nSpecies)) {
      ...
      for (a in (info[s,'faf']:info[s,'la'])) {
        for (q in 1:lq) if (a >1 | q>=recSeason ) {
          if (y %in% zeroCatchYear[[s]]) {
            ...
            if (fModel==2) if (a >=fSepar)  FisQ[[s]][y,q,a]<-FisQ[[s]][y,q,a] * exp(logSeparF[keyLogSeparF[[s]][y,a]])
    ...
    #####  Now take care of N
    if (inclProcessNoise[s] %in% c(1,2,3)) {
      nvar <- outer(1:stateDimN[s], 1:stateDimN[s],  # SKAL LAVES OM
                  function(i,j) (i==j)*varLogN[ keyVarLogN[s,i]])
      
      # nvar SKAL PASSES TIL SÅ DEN PASSER TIL ANTAL ALDRE
    }
    if (inclProcessNoise[s] %in% c(1,2)) {
    ...
    } else { # recruitment as parameter
      
    }    
   
    
    #  remaining Years
    if (inclProcessNoise[s] %in% c(1,2,3)) for (i in seq.int(2L,timeSteps)) { 
      if (inclProcessNoise[s] %in% c(1,2)) predN[[s]][1,i]<-SSB_R(s,y=i); # recruits
     
       # older ages
      if (inclProcessNoise[s] %in% c(1,3)) for (j in seq.int(2L,stateDimN[s])) predN[[s]][j,i]<-logN[[s]][j-1,i-1] - sum(FisQ[[s]][i-1,,j-1]) - sum(MM[[s]][i-1,,j-1]) 
      ...
      llh <- dmvnorm(logN[[s]][,i], predN[[s]][,i], nvar, log=TRUE) ## N-Process likelihood SKAL PASSES TIL ANTAL ALDRE
      ans <- ans -llh
      nlls[s,"N"]<-  nlls[s,"N"] -llh 
    }


  # calculate N by quarter
    for(i in seq_len(timeSteps)) {
      for(j in seq_len(stateDimN[s])) {
        ...
        if (j >1 | recSeason==1 ) {  # first age might not have enter the model yet
          ...
          if (keyLogFsta[s,j]>0) Chat[[s]][j,i] <- exp(logNbarq[[s]][i,fq,j])*FisQ[[s]][i,fq,j]
        ...
        if (j==1 & recSeason>1) {
           ...
           if (keyLogFsta[s,j]>0)  {Chat[[s]][j,i] <-  exp(logNbarq[[s]][i,recSeason,j])* FisQ[[s]][i,recSeason,j]
           } else { Chat[[s]][j,i]<-0.1  }
        }
        ...
        if (nSeasons>1) for (q in 2:lq) if (j >1 | q>recSeason ) {
          ...
          if (keyLogFsta[s,j]>0)  {Chat[[s]][j,i] <- Chat[[s]][j,i]+ exp(logNbarq[[s]][i,q,j]) * FisQ[[s]][i,q,j]}  else { Chat[[s]][j,i]<-0.1  }
        }
      }
    }
