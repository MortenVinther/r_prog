...
#  dir=data.path; sms.dat='rsms.dat';seasFfrom=c('F','catch')[2];multi=FALSE
 ...
 #TEST    dir=data.path; sms.dat='rsms.dat'; seasFfrom<-'catch';seasFfrom=c('F','catch')[2]; multi=TRUE
...
b1<-filter(b,annualC==1) %>%  dplyr::select(year, species.n, quarter, sub_area,age, annualC) %>% mutate(seasFprop=0.25) %>%
