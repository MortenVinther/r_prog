...
partial_M2<-function(
  ...
  if (include_M1) {
    preys<-sort(unique(a$Prey))
    m1<-Read.summary.data() %>% mutate(Predator='M1',Predator.age=99,Predator.no=0,Prey=Species,Prey.no=Species.n,Prey.age=Age,Part.M2=M1) %>%
    select(Predator,Prey, Year, Quarter, Predator.no, Predator.age, Prey.no, Prey.age,Part.M2) %>% filter(Part.M2>0 &Prey %in% preys & Year<=last.y) %>% as_tibble()
    a<-rbind(a,m1)
  }
  ...
  do.plot<-function(){
    ...
    if (file.exists(file)) {
      ...
      if (include_M1) s$newno<-s$newno+1
      pformat<-unique(subset(s,newno<100,select= -old))
    ...
    by(x,list(x$Prey.age,x$Prey),function(x) {
     # x<-subset(x,Prey.age==1)
        ...
        if ((i %% (nox*noy-1))==0) {
          ...
          if (include_M1) mm<-'M' else mm<-'M2'
          filename<-paste0("Partial_",mm,"_annual_",x[1,'Prey'])
