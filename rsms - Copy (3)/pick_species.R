pick_species<-function(ps=c(1L), pso=c(0L),inp,smsConf=0L) {
 ...
 #test_multi  ps<-my.ps; pso=my.pso; inp=inp_all; smsConf=1L ;ps; pso
  ...
  d$inclProcessNoise<-data$inclProcessNoise[ps]
  ...
  d$logNrecruitParamfromTo<-cutFromTo(data$logNrecruitParamfromTo)
  ...
  if (smsConf==0) {  # single species
    p$vulnera<-rep(0.0,0)
    p$overlapP<-rep(0.0,0)
    p$logStomObsVar<-rep(0.0,0)
  }
