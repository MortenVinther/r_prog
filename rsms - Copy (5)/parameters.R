extractParameters<-function(sdrep,myMap,data) {
  ...
  if ('logSdLogObsCatch' %in% parName) keys<- rbind(keys,transKey(key=data$keyVarObsCatch, pp='logSdLogObsCatch',type='species'))
  ...
  mk<-mk %>% mutate(s=if_else(type %in% c('species','predator'),match(Var1,data$allSpNames),-9))
