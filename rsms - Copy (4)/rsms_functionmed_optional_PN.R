func <- function(parameters) {
  ...
  for (s in seq_len(nSpecies)) {
    ...
    for (y in seq_len(nYears)) {
       ...
       ssb[s,y]<- sum(exp(logNq[[s]][y,spawnSeason,]) *propMat[[s]][y,spawnSeason,]*stockMeanWeight[[s]][y,spawnSeason,]) 
    }
   ...
   for (i in seq_len(timeSteps)) {
      for (j in seq_len(la[s])) {
        ...
        # first quarter     
        if (j >1 | recSeason==fq ) { 
          #logNq[[s]][i,fq,j]<-logN[[s]][j,i]  # SKAL VÆÆÆÆÆÆÆÆÆÆÆÆK
        ...
        if (j==1 & recSeason>fq) {
          #logNq[[s]][i,recSeason,j]<-logN[[s]][j,i]   # overflødig SKAL VÆÆÆÆÆÆÆÆÆÆÆÆK
     ...
     #   doProcessN_none
        ...
        if ( ((doProcessN_any[s] & !doProcessN_old[s]) | doProcessN_none[s]) & i<nYears) {
          if (j <la[s]) {
             ...
             if ( maxAgePlusGroup) logNq[[s]][i+1,fq,j]<- log( exp(logNq[[s]][i,lq,j]-Zq[[s]][i,lq,j]) + exp(logNq[[s]][i,lq,j-1]))
          }
        }
