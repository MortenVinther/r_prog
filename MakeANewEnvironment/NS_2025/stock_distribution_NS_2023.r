...
#Mackerel
a<-read.csv(file=file.path(root,exchangeDir,'ByStock','MAC','catchPerAreaWGWIDE2023.csv'),skip=6)
head(a)

a$Prop4_3a<-a$cat34/a$catch
x11(); plot(a$Year,a$Prop4_3a)
...
b<-t(b)/1000
b
X11();barplot(b,legend = rownames(b),ylab="Catches (1000 tonnes)",args.legend = list(x = "topleft"))

x11(); plot(a$Year,a$Prop4_3a,xlab='',ylab='Proportion within area 3a and 4',type='b',lwd=2)
...
dd<-filter(d,year==2019) %>% arrange(quarter,age)
d<-rbind(d,
  dd %>% mutate(year=2020),
  dd %>% mutate(year=2021),
  dd %>% mutate(year=2022)
