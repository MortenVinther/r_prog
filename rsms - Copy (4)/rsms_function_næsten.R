...
func <- function(parameters) {
  ...
  #function for Fishing mortality at age (annual)
    ...
    for(i in seq_len(timeSteps)) {
      for(j in seq_len(stateDimN[s])) {
        ...
        if (j >1 | recSeason==1 ) {  # first age might not have enter the model yet
          ...
          if (keyLogFsta[s,j]>0) Chat[[s]][j,i] <- exp(logNbarq[[s]][i,fq,j]+fiMo(s,i,j,expIt=FALSE)+logSeasFprop[[s]][i,fq,j])
        ...
        if (j ==1 & recSeason>1) {
           ...
           if (keyLogFsta[s,j]>0)  {Chat[[s]][j,i] <-  exp(logNbarq[[s]][i,recSeason,j]+fiMo(s,i,j,expIt=FALSE)+logSeasFprop[[s]][i,recSeason,j])
           } else { Chat[[s]][j,i]<-0.1  }
        }
        ...
        if (nSeasons>1) for (q in 2:lq) if (j >1 | q>recSeason ) {
          ...
          if (keyLogFsta[s,j]>0)  {Chat[[s]][j,i] <- Chat[[s]][j,i]+ exp(logNbarq[[s]][i,q,j]+fiMo(s,i,j,expIt=FALSE)+logSeasFprop[[s]][i,q,j])}  else { Chat[[s]][j,i]<-0.1  }
        }
      }
    }
  ...
  fleets<-keySurvey.overview[keySurvey.overview[,'s']==s,'f']
    ...
    if (surveyType[fl]==1) {
      ...
      for (a in mina[fl]:maxa[fl]) {
        ...
        obs.no<-keysA[,'obs.no']
        predSurveyObs[obs.no]<- fiMo(s,flYearss,a,expIt=FALSE)+logSeasFprop[[s]][flYears,q,a] + logCatchability[keyCatchability] 
