#
### init: Run: init.R in the SMS directory and then _init_rsms.R
...
if (TRUE) {  # transform  SMS data into RSMS format 
  switch(my.stock.dir,
   ...
   "rsms_SAN-area-1r" = inp_all<-make_rsms_data(dir=data.path,sms.dat='rsms.dat',seasFfrom=c('F','catch')[2],multi=FALSE),
...
##### use simulated catch numbers
# inp_all<-simCatchN(inp=inp_all, maxVar=1.0 ^2)
...
#my.ps<-c(1L,7L,8L)
#my.ps<-c(1L,2L,3L,4L,6L,7L,8L,9L,10L,11L)
my.ps<-c(1L)
my.pso<-c(0L)
#my.pso<-13L:27L
...
#  parameters<-as.list(sdrep, what="Est")  #parameters and random effects
...
# summary(sdrep,select = c("all", "fixed", "random", "report")[2])

a<-extractParameters(sdrep,myMap,data)[[2]]
  print(a,n=300)
#  arrange(a,desc(abs(gradient)))
#  arrange(a,desc(is.na(estimate.sd)))
#  print(filter(a,name=='logSdLogN'),n=30)
#  print(filter(a,name=='logSdLogFsta'),n=30)
#print(filter(a,name=='logSdLogObsSurvey'),n=100)
print(filter(a,s==3),n=100)
...
summary(filter(sms[['rep']]$resSummary,s==1))

filter(sms$rep$res,year==1980)
...
if (FALSE) plotCompareRunSummary(Type=c("compSummaryConf","compSummary")[1],showSpecies=1:12,
