compare_runs_stock_rec<-function(dirs,labels,first.year.on.plot=1975,last.year.on.plot=2020,incl.sp="all",
                                 ...
                                 include.year.labels=TRUE,incl_not_used=FALSE,run.ID='SBB_rec', 
                                 paper=FALSE,facSSB=1000,facRec=1000000,
                                 compare.dir=data.path,verbose=FALSE,writeData=TRUE,combinePlot=FALSE) {
   ...
   SSB_R<-function(SSB,model,alfa,beta,delta=0.5,info1,info2,std) {
      ...
      else if (model==4) {for (ii in (1:length(x))) y[i]<-exp(alfa)*min(x[i],exp(beta),na.rm=T)}
      ...
      else if (model==100) {for (ii in (1:length(x))) y[ii]<-exp(alfa)*min(x[ii],beta,na.rm=T)}
      return(y)
   }
   ...
   plotvar<-function(sp,dat,plotType='p',addModel=FALSE) {
      if ((gi %% (nox*noy))==0  | gi==0) {
           ...
           if (makeAllGraphs) ddir<-compare.dir else ddir<-data.path
            newplot(dev,nox,noy,filename=filename,Portrait=F,w8=w8,w11=w11,dir=ddir);
             par(mar=c(4,5,3,1))   # c(bottom, left, top, right)
      }
      
      for (l in labels ) {
         xx<-subset(dat,label==l & !is.na(Rec))
         xx$incl<-xx$Year>=SSB.R.year[1,sp] & xx$Year<=SSB.R.year[2,sp] & xx$used
         #print(as.character(sprintf("%02.0f",xx$Year%%100)))
         outData<<-rbind(outData,xx)
        
         if (!incl_not_used) xx<-subset(xx,incl)
         #print("HEJ 1")
         #print(xx)
         #print("HEJ 2")
         #print(c(0,max(xx$SSB,na.rm=TRUE)))
         #print("HEJ 3")
         plot(xx$SSB,xx$Rec,xlab="SSB (kt)",ylab="Recruits (billions)",main=ifelse(combinePlot,sp,paste(l,sp)), lwd=2,xlim=c(0,max(xx$SSB,na.rm=TRUE)), ylim=c(0,max(xx$Rec,na.rm=TRUE)*1.1),type=plotType )
         ...
         x<-seq(0,max(xx$SSB,na.rm=T), by=max(xx$SSB,na.rm=T)/100)*facSSB
   ...
   cleanup()
   if (combinePlot) {
     filename<-paste0("compare_",run.ID,'_','all_sp')
     #print(filename)
     if (makeAllGraphs) ddir<-compare.dir else ddir<-data.path
     
     newplot(dev,nox,noy,filename=filename,dir=ddir,Portrait=F,w8=w8,w11=w11);
     par(mar=c(4,5,3,1))   # c(bottom, left, top, right)
   }
   ...
   if (writeData) {
     outData<-dplyr::arrange(outData,Species.n,label,Year) 
     write.csv(outData, file=file.path(data.path,'stock_rec.csv'),row.names = FALSE)
   }
   if (dev=='png') cleanup()
}
