    ...
    switch(x,
           ...
           "combined.catches"  ={if (nice) {
             ...
             cat("## use seasonal or annual catches",inpOnly,  "(option combined.catches)\n",
                 "#    0=seasonal catch input with seasonal proportions of annual F at age derived from seasonal catch N / sum(all seasonal catch N)\n",
                 "#    1=annual catches input with seasonal proportions of annual F at age from number of seasons (1/nseasons)\n",
                 "#    2=annual catches input with seasonal proportions of annual F at age from file proportion_of_annual_f.dat (not implemented yet)\n",
                 "#    3=seasonal catch input with estimation of seasonal F from separable F-model (not implemented yet)\n",
                 file=file,append=T,sep="")   
             if (expand) wr.vector.expand(slot(control,x),VPA.species) else wr.vector.nice(slot(control,x),VPA.species)
