...
if (FALSE) {
  str(myRep,1)
  res<-myRep$res; res[is.na(res$FF),'FF']<-0.0; summary(res)
  resAnnual<-myRep$resAnnual; summary(resAnnual)
  resSummary<-myRep$resSummary; summary(resSummary)
}
...
##load(file=file.path(data.path,paste0("SMS_old",".Rdata")),verbose=T)
#xtabs(N~paste(s,species)+age,data=sms$rep$res)

load(file='ICES_single_sp.Rdata',verbose=TRUE)
...
if (FALSE) plotCompareRunSummary(Type=c("compSummaryConf","compSummary")[2],showSpecies=1:12,
                                 ...
                                 outFormat=c('screen','pdf','png')[1],
                                 longSpNames=FALSE, fileLabel='SS_ICES_')
